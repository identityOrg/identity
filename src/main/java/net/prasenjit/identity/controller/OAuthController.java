...
import net.prasenjit.identity.model.ui.Tuple;
...
public class OAuthController {
    ...
    private String generateResponse(HttpSession httpSession, Model model, AuthorizationResponse response,
        ...
        if (response.impliedResponseMode() == ResponseMode.FORM_POST) {
            ...
            List<Tuple> paramList = responseMap.entrySet().stream()
                    .flatMap(entry -> entry.getValue().stream()
                            .map(v -> new Tuple<>(entry.getKey(), v)))
                    .collect(Collectors.toList());
            model.addAttribute("paramList", paramList);
            model.addAttribute("redirect_uri", response.getRedirectionURI());
